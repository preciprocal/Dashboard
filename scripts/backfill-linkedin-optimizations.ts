// scripts/backfill-linkedin-optimizations.ts
// Phase 3a: copy Firestore's `linkedinOptimizations` collection into
// Postgres `linkedin_optimizations`, remapping the legacy Firebase uid to
// the real Supabase auth UUID. Rows with no mappable user are skipped.
//
// Usage:
...
async function main() {
  ...
  const [snap, uidMap] = await Promise.all([
    db.collection("linkedinOptimizations").get(),
    buildFirebaseUidToSupabaseIdMap(),
  ]);
  console.log(`Found ${snap.size} Firestore linkedinOptimizations docs, ${uidMap.size} legacy uid mappings`);
