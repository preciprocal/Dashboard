      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-start gap-3 flex-1">
            <div className={`p-2 rounded-lg ${getTypeColor(template.type)} flex-shrink-0`}>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <span className={`px-2.5 py-1 rounded-md text-xs font-medium ${getDifficultyColor(template.difficulty)}`}>
            {template.difficulty}
          </span>
          <span className={`px-2.5 py-1 rounded-md text-xs font-medium ${getTypeColor(template.type)}`}>
            {template.type}
          </span>
          {template.tags && template.tags.includes("Popular") && (
            <span className="px-2.5 py-1 rounded-md text-xs font-medium text-purple-400 bg-purple-500/10">
              Popular
            </span>
          )}
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {template.techstack.slice(0, 4).map((tech, index) => (
              <span
                key={index}
                className="px-2 py-0.5 rounded text-xs font-medium text-blue-400 bg-blue-500/10"
              >
                {tech}
              </span>
            ))}
            {template.techstack.length > 4 && (
              <span className="px-2 py-0.5 rounded text-xs font-medium text-slate-400 bg-slate-600/10">
                +{template.techstack.length - 4}
              </span>
            )}
          </div>
        ...
        {/* Stats Grid */}
        <div className="grid grid-cols-3 gap-3 mb-5 py-3 border-t border-white/5">
        ...
        {/* Action Button */}
        <button className="w-full glass-button-primary hover-lift py-2.5 px-4 rounded-lg font-medium text-sm flex items-center justify-center gap-2">
          Use Template
        </button>
      </div>
    </div>
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="glass-card hover-lift">
          <div className="text-center p-12">
            ...
            <p className="text-slate-400 mb-6">Please log in to view interview templates</p>
            <Link 
              href="/sign-in"
          ...
          <div className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-yellow-400 text-sm mb-2">{dataError}</p>
                <button
