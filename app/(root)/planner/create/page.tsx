...
export default function CreatePlanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.role.trim()) { setError('Please enter the job role'); return; }
    if (!formData.interviewDate) { setError('Please select your interview date'); return; }
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-2.5">
                  {(['beginner', 'intermediate', 'advanced'] as SkillLevel[]).map(level => (
                    <button key={level} type="button" onClick={() => setFormData({ ...formData, skillLevel: level })}
                      className={`px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg font-medium text-xs sm:text-sm transition-all ${formData.skillLevel === level ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white' : 'bg-slate-800/50 text-slate-300 hover:bg-slate-800 border border-slate-700'}`}>
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 sm:p-6">
            ...
            {formData.focusAreas.length > 0 && (
              <div className="mb-3 flex items-center gap-2 text-sm">
                ...
                <span className="px-2 py-0.5 bg-blue-500/20 border border-blue-500/30 rounded text-blue-400 font-medium">{formData.focusAreas.length}</span>
              </div>
            )}
            <div className="space-y-6 max-h-96 overflow-y-auto scrollbar-hide">
              {Object.entries(filteredCategories).length > 0 ? (
                Object.entries(filteredCategories).map(([category, areas]) => (
                  <div key={category}>
                    <h3 className="text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2">
                      <div className="h-px flex-1 bg-slate-700" /><span>{category}</span><div className="h-px flex-1 bg-slate-700" />
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                      {areas.map(area => (
                        <button key={area} type="button" onClick={() => toggleFocusArea(area)}
                          className={`px-3 py-2 rounded-lg text-xs font-medium transition-all text-left ${formData.focusAreas.includes(area) ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'bg-slate-800/50 text-slate-300 hover:bg-slate-800 border border-slate-700'}`}>
                          {area}
                        </button>
                      ))}
                    </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-400 text-sm">No focus areas found matching &quot;{searchQuery}&quot;</p>
                  <button type="button" onClick={clearSearch} className="mt-3 text-blue-400 hover:text-blue-300 text-sm font-medium">Clear search</button>
                </div>
              )}
            </div>
          </div>
