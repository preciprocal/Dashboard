...
function calcStats(list: Resume[]): ResumeStats {
  const withFeedback = list.filter(r => r.feedback);
  if (withFeedback.length === 0) return { averageScore: 0, totalResumes: list.length, improvementTips: 0 };

  const avgScore = withFeedback.reduce((s, r) => s + (r.feedback?.overallScore || 0), 0) / withFeedback.length;
  ...
  return { averageScore: Math.round(avgScore), totalResumes: list.length, improvementTips: totalTips };
}
...
export default function ResumeDashboard() {
  ...
  const [stats, setStats] = useState<ResumeStats>({ averageScore: 0, totalResumes: 0, improvementTips: 0 });
  ...
  // Prevent re-fetching on every render cycle
  ...
  const loadResumes = useCallback(async (forceRefresh = false): Promise<void> => {
    if (!user) return;
    ...
    try {
      ...
      setLoadingStep(3);
      setStats(calcStats(userResumes));
  ...
  useEffect(() => {
    if (user && !hasFetched.current) loadResumes();
  }, [user, loadResumes]);
  ...
  const filteredResumes = useMemo(() => {
    ...
    switch (sortFilter) {
      ...
      case 'needs-improvement':
        return filtered.filter(r => r.feedback && r.feedback.overallScore < 70);
      case 'recent':
      case 'all':
      default:
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh] px-4">
        <div className="glass-card p-10 text-center max-w-sm w-full">
          ...
          <Link href="/sign-in" className="inline-flex items-center justify-center gap-2 w-full px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-sm font-semibold hover:from-indigo-400 hover:to-purple-400 transition-all duration-150">
            Go to sign in
          </Link>
        </div>
      </div>
        ...
        <div className="glass-card p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <button onClick={handleRetryError} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.05] border border-white/[0.08] text-xs font-medium text-slate-300 hover:text-white transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="glass-card p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="w-8 h-8 bg-blue-500/10 rounded-lg flex items-center justify-center"><FileText className="w-4 h-4 text-blue-400" /></div>
            ...
            <div className="glass-card p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="w-8 h-8 bg-amber-500/10 rounded-lg flex items-center justify-center"><Zap className="w-4 h-4 text-amber-400" /></div>
          ...
          <div className="glass-card p-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-[11px] text-slate-600 mt-0.5">{filteredResumes.length} of {resumes.length} resumes</p>
              </div>
              <div className="flex items-center gap-2">
                <div className="relative filter-dropdown">
                  <button type="button" onClick={() => setShowFilterMenu(!showFilterMenu)}
                    ...
                    <span className="flex items-center gap-1.5"><Filter className="w-3.5 h-3.5" />{getFilterLabel(sortFilter)}</span>
                  ...
                  {showFilterMenu && (
                    <div className="absolute right-0 top-full mt-2 w-52 bg-[#0d1526]/98 border border-white/[0.08] rounded-xl shadow-[0_16px_40px_rgba(0,0,0,0.5)] z-20 overflow-hidden">
                      {(['all', 'high-scores', 'needs-improvement', 'recent'] as SortOption[]).map(option => (
                        <button key={option} type="button" onClick={() => { setSortFilter(option); setShowFilterMenu(false); }}
                          className={`w-full px-4 py-2.5 text-left text-xs font-medium transition-colors ${sortFilter === option ? 'bg-indigo-500/15 text-indigo-300' : 'text-slate-400 hover:text-slate-200 hover:bg-white/[0.04]'}`}>
                ...
                <div className="flex items-center gap-0.5 p-1 bg-white/[0.03] border border-white/[0.07] rounded-lg">
                  <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded transition-all duration-150 ${viewMode === 'grid' ? 'bg-white/[0.10] text-white' : 'text-slate-600 hover:text-slate-300'}`} aria-label="Grid view"><LayoutGrid className="w-3.5 h-3.5" /></button>
                  <button onClick={() => setViewMode('list')} className={`p-1.5 rounded transition-all duration-150 ${viewMode === 'list' ? 'bg-white/[0.10] text-white' : 'text-slate-600 hover:text-slate-300'}`} aria-label="List view"><List className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            </div>
          </div>
          ...
          {filteredResumes.length > 0 ? (
            <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4' : 'space-y-3'}>
              {filteredResumes.map((resume, index) => (
                <div key={resume.id} className="opacity-0 animate-fadeIn"
                  style={{
                    animationDelay: `${index * 100}ms`,
                    animationFillMode: 'forwards',
                    ...(deletingId === resume.id ? { opacity: 0, transform: 'scale(0.95)', transition: 'opacity 300ms ease, transform 300ms ease', pointerEvents: 'none' as const } : {}),
                  }}>
                  <ResumeCard resume={resume} viewMode={viewMode} onDelete={handleDelete} />
                </div>
              ))}
            </div>
          ) : (
            <div className="glass-card p-12 text-center">
              <div className="w-12 h-12 bg-white/[0.03] border border-white/[0.07] rounded-2xl flex items-center justify-center mx-auto mb-4"><FileText className="w-5 h-5 text-slate-600" /></div>
              ...
              <button onClick={() => setSortFilter('all')} className="text-xs text-slate-400 hover:text-white underline transition-colors">Clear filter</button>
            </div>
          )}
        </div>
      ) : !resumesError ? (
        <div className="glass-card p-12 text-center">
          <div className="w-16 h-16 bg-white/[0.03] border border-white/[0.07] rounded-2xl flex items-center justify-center mx-auto mb-6"><FileText className="w-7 h-7 text-slate-600" /></div>
          <h3 className="text-xl font-bold text-white mb-2">Welcome to Resume Analysis</h3>
          <p className="text-sm text-slate-500 max-w-md mx-auto mb-8 leading-relaxed">Get AI-powered insights, ATS optimization, and personalized recommendations to improve your resume</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8 max-w-3xl mx-auto">
            {[
              ...
              <div key={label} className="text-center">
                <div className={`w-10 h-10 bg-${color}-500/10 rounded-lg flex items-center justify-center mx-auto mb-2`}><Icon className={`w-5 h-5 text-${color}-400`} /></div>
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <SeeExampleButton serviceId="resume" className="!px-5 !py-2.5 !text-sm !font-semibold" />
          </div>
