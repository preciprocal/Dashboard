...
export default function CoverLetterGeneratorPage() {
  ...
  useEffect(() => {
    console.log('🔍 [COVER-LETTER] Setting up LinkedIn data listener...');
    
    const handleMessage = (event: MessageEvent) => {
      if (event.origin !== window.location.origin) return;
      if (event.data.type === 'LINKEDIN_JOB_DATA_RESPONSE') {
        console.log('📨 [COVER-LETTER] Received response from content script');
        if (event.data.data) {
          ...
          if (timestamp && Date.now() - timestamp < 60000) {
            console.log('✅ LinkedIn job data:', jobData.title);
          ...
          console.log('ℹ️ No LinkedIn job data available');
        }
      }
    };
    ...
    setTimeout(() => {
      console.log('📤 Requesting LinkedIn job data from content script...');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.success && data.coverLetter) {
        ...
        // ── Notification: cover letter generated ──
        if (user?.uid) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ── Notification: cover letter saved ──
      if (user?.uid) {
        const target = companyName.trim()
          ? `${jobRole.trim()} at ${companyName.trim()}`
          : jobRole.trim();
        ...
        await NotificationService.createNotification(
          ...
          {
            actionUrl: `/cover-letter/${docRef.id}`,
  ...
  if (!user) {
    console.log('❌ [COVER-LETTER] No user found, LayoutClient should redirect');
    return null;
  }
