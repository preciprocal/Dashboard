...
const ProfileSaved: React.FC<ProfileSavedProps> = ({
  ...
  const isTemplate = (item: Template | Blog): item is Template => {
    return 'type' in item || 'role' in item || 'difficulty' in item;
  };

  const getItemTitle = (item: Template | Blog): string => {
    if (isTemplate(item)) {
      return item.role || item.title || 'Untitled';
    }
    return item.title || 'Untitled';
  };

  const getItemMeta = (item: Template | Blog): string => {
    if (isTemplate(item)) {
      return item.category || 'Template';
    }
    return item.author ? `By ${item.author}` : 'Article';
  };

  const getItemDuration = (item: Template | Blog): string => {
    if (isTemplate(item)) {
      return item.duration || item.readTime || '30 min';
    }
    return item.readTime || '5 min';
  };
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center space-x-4 mt-2 text-xs text-gray-500 dark:text-gray-500">
                              ...
                              <span className="flex items-center">
                                ...
                                {template.uses || Math.floor(Math.random() * 50) + 10} uses
                              </span>
                              <span className="flex items-center">
