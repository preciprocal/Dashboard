...
const VERDICT_STYLES: Record<BenchmarkDimension['verdict'], { bar: string; badge: string; label: string }> = {
  strong:      { bar: 'from-emerald-500 to-teal-500',  badge: 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400',  label: 'Strong'      },
  competitive: { bar: 'from-amber-400 to-yellow-500',  badge: 'bg-amber-500/10 border-amber-500/20 text-amber-400',        label: 'Competitive' },
...
function BenchmarkLoadingState() {
  ...
  useEffect(() => {
    timerRef.current = setInterval(() => {
      setVisible(false);
      setTimeout(() => { setFactIndex(i => (i + 1) % BENCHMARK_FACTS.length); setVisible(true); }, 400);
    }, 4000);
    ...
    <div className="bg-slate-800/40 border border-white/[0.06] rounded-xl overflow-hidden cursor-pointer hover:border-white/10 hover:bg-slate-800/60 transition-all"
      ...
      <div className="p-3.5">
        ...
        <div className="flex items-center gap-3 text-[10px] text-slate-500 flex-wrap">
          ...
          <span className="flex items-center gap-1"><span className="w-2 h-px bg-violet-400/80 inline-block" />Hired: {dim.hiredMedian}</span>
          <span className="flex items-center gap-1"><span className="w-2 h-px bg-teal-400/60 inline-block" />Top 10%: {dim.topTen}</span>
          <span className="ml-auto text-slate-500">You: {dim.userPercentile}th pct</span>
        </div>
      </div>
      {expanded && (
