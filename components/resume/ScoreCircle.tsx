...
export default function ScoreCircle({ score, size = 'medium', showLabel = false, className = '' }: ScoreCircleProps) {
  ...
  const getScoreColor = (s: number) => {
    if (s >= 80) return { text: 'text-green-600 dark:text-green-400',  stroke: 'stroke-green-600 dark:stroke-green-400'  };
    if (s >= 60) return { text: 'text-yellow-600 dark:text-yellow-400',stroke: 'stroke-yellow-600 dark:stroke-yellow-400' };
    ...
    <div className={`relative ${width} ${height} flex flex-col items-center ${className}`}>
      <svg className={`${width} ${height} transform -rotate-90`} viewBox="0 0 80 80">
        ...
        <circle cx="40" cy="40" r={radius} fill="transparent"
          ...
          style={{ animation: 'score-circle-draw 1.5s ease-out forwards' }} />
      </svg>
      ...
      {showLabel && (
        <span className={`mt-1 text-xs font-medium ${colors.text}`}>{getScoreLabel(score)}</span>
      )}

      {/*
        FIX: <style jsx> replaced with dangerouslySetInnerHTML.
        The keyframe uses JS-interpolated values so we build the string directly.
        styled-jsx is not installed in this project.
      */}
